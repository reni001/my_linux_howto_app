...
def handle_delete(app, name, popup):
    success, reason = delete_category_safe(name)

    if success:
        popup.dismiss()
        show_category_dialog(app)
        return
...
def show_category_dialog(app):
    ...
    if not cats:
        ...
        for cat in cats:
            ...
            icon_name = cat.get("icon") or "howtolinux-icon.png"
            ...
            usage = count_category_usage(cat["name"])
            ...
            btn_edit.bind(
                on_release=lambda instance, item=dict(cat): show_edit_category_dialog(app, popup, item)
            )
            ...
            # ✅ check usage
            is_used = is_category_used(cat["name"])

            btn_del.disabled = is_used
            btn_del.opacity = 0.3 if is_used else 1

            btn_del.bind(
                on_release=lambda instance, name=cat["name"]: handle_delete(app, name, popup)
            )
    ...
    btn_add.bind(on_release=lambda x: show_edit_category_dialog(app, popup, None))
...
# EDIT / ADD DIALOG
# ----------------------------
def show_edit_category_dialog(app, parent_popup, cat):
    ...
    edit_mode = cat is not None

    old_name = cat["name"] if cat else ""
    old_icon = cat["icon"] if cat else "howtolinux-icon.png"
    ...
    def do_save(instance):
        ...
        if edit_mode:
            apply_category_change(app, old_name, new_name, new_icon)
        else:
            upsert_category(new_name, new_icon)
